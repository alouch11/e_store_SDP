...
class ProductRepo {
  ...
  Future<ApiResponse> getLatestProductList(BuildContext context, String offset, ProductType productType, String? title) async {
    late String endUrl;
     ...
     if(productType == ProductType.newArrival){
    ...
    title = getTranslated('top_selling', context);
    }
  ...
  Future<ApiResponse> getAlternativeProductList(String id) async {
    try {
      final response = await dioClient!.get('${AppConstants.alternativeProductUri}$id?guest_id=1');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
...
///products transactions list
  ...
  Future<ApiResponse> getProductTransactionsList(int offset,String id) async {
    try {
      final response = await dioClient!.get("${AppConstants.productTransactionsList}$id?guest_id=1&limit=10&offset=$offset");
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
