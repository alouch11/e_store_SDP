...
import 'package:flutter_spareparts_store/features/category/domain/model/find_what_you_need.dart';
import 'package:flutter_spareparts_store/features/product/domain/model/most_demanded_product_model.dart';
...
int selectedProductTypeIndex = 0;
 void changeTypeOfProduct(ProductType type, String? title, {int index = 0}){
  ...
  List<Product>? _alternativeProductList;
  List<Product>? get alternativeProductList => _alternativeProductList;

  void initAlternativeProductList(String id, BuildContext context) async {
    ApiResponse apiResponse = await productRepo!.getAlternativeProductList(id);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _alternativeProductList = [];
      apiResponse.response!.data.forEach((product) => _alternativeProductList!.add(Product.fromJson(product)));
    } else {
      ApiChecker.checkApi( apiResponse);
    }
    notifyListeners();
  ...
  void removePrevAlternativeProduct() {
    _alternativeProductList = null;
  }
...
/*
  ...
  List<ProductsTransactionsModel>? _producttransactionsDetails;
  List<ProductsTransactionsModel>? get producttransactionsDetails => _producttransactionsDetails;

  Future<ApiResponse> getProductTransactions(String productID, BuildContext context) async {
    _producttransactionsDetails = null;
    ApiResponse apiResponse = await productRepo!.getProductTransactionsList(productID);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _producttransactionsDetails = null;
      _producttransactionsDetails = [];
      apiResponse.response!.data.forEach((product) => _producttransactionsDetails!.add(ProductsTransactionsModel.fromJson(product)));
    } else {
      ApiChecker.checkApi( apiResponse);
    }
    notifyListeners();
...
*/
  ...
  ProductsTransactionsModel? producttransactionsDetails;
  Future<void> getProductTransactionsList(int offset, String id) async {
    if(offset == 1){
      producttransactionsDetails = null;
    }
    ApiResponse apiResponse = await productRepo!.getProductTransactionsList(offset,id );
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      if(offset == 1){
        producttransactionsDetails = ProductsTransactionsModel.fromJson(apiResponse.response?.data);

      }else {
        producttransactionsDetails!.transactionsDetails?.addAll(ProductsTransactionsModel.fromJson(apiResponse.response?.data).transactionsDetails!);
        producttransactionsDetails!.offset = ProductsTransactionsModel.fromJson(apiResponse.response?.data).offset;
        producttransactionsDetails!.totalSize = ProductsTransactionsModel.fromJson(apiResponse.response?.data).totalSize;
      }
