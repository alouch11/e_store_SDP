...
import 'package:flutter_spareparts_store/features/notification/widget/notification_dialog.dart';
import 'package:provider/provider.dart';
import 'package:shimmer/shimmer.dart';

class NotificationScreenRead extends StatefulWidget {

  const NotificationScreenRead({super.key,  });
  @override
  State<NotificationScreenRead> createState() => _NotificationScreenReadState();
}

class _NotificationScreenReadState extends State<NotificationScreenRead> {
  ScrollController scrollController = ScrollController();

  @override
   initState(){
        Provider.of<NotificationProvider>(context, listen: false).getNotificationList(1,'read');
        Provider.of<NotificationProvider>(context, listen: false).notificationType='read';
  ...
  Widget build(BuildContext context) {
    return Consumer<NotificationProvider>(
        builder: (context, notification, child) {
          ...
          RefreshIndicator(
            onRefresh: () async {
              await Provider.of<NotificationProvider>(context, listen: false).getNotificationList(1,'read');
            },
