...
import 'package:flutter_spareparts_store/features/search/widget/search_type_button.dart';
...
import 'package:flutter_spareparts_store/features/search/widget/partial_matched.dart';
import 'package:flutter_spareparts_store/features/search/widget/search_product_widget.dart';
import 'package:provider/provider.dart';

import '../NFC/notifier/nfc_notifier.dart';
import '../NFC/widget/dialogs.dart';
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    String searchByLocation =Provider.of<ProfileProvider>(context, listen: false).userInfoModel!.searchByLocation ?? '';
    ...
    return Column(children: [
      if(searchByLocation == 'yes')
            Padding(padding: const EdgeInsets.all(Dimensions.paddingSizeLarge),
              child: Row(children: [
                SearchTypeButton(text: getTranslated('by_item', context), index: 'byitem'),
                  const SizedBox(width: Dimensions.paddingSizeSmall),
                  SearchTypeButton(text: getTranslated(''
                      'by_location', context), index: 'bylocation'),
                                ...
                                children: [for (int index =0; index < searchProvider.historyList.length; index++)
                                  Padding(padding:  const EdgeInsets.symmetric(vertical: Dimensions.paddingSizeSmall),
                                    child: Container(decoration: BoxDecoration(
                                      ...
                                      child: InkWell(onTap: () => searchProvider.searchProduct( query : searchProvider.historyList[index],searchtype: searchProvider.selectedType, offset: 1),
                            ...
                            children: [for (int index = 0; index < popularTagProvider.configModel!.popularTags!.length; index++)
                              Padding(padding:  const EdgeInsets.symmetric(vertical: Dimensions.paddingSizeSmall),
                                child: Container(decoration: BoxDecoration(
                                  ...
                                  padding: const EdgeInsets.symmetric(vertical: Dimensions.paddingSizeSmall-3, horizontal: Dimensions.paddingSizeSmall),
                                  margin: const EdgeInsets.only(right: Dimensions.paddingSizeSmall),
                                  child: InkWell(onTap: () => Provider.of<SearchProvider>(context, listen: false).searchProduct(query : popularTagProvider.configModel!.popularTags![index].tag??'',searchtype: searchProvider.selectedType, offset: 1),
